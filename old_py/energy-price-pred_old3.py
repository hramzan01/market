...
def process_df(file, previous_days=6*30, split_ratio=0.9):
...
def ml_model(train, forecast_days=7, seasonality_mode = 'multiplicative', year_seasonality_mode=4):
...
def pred(df_price, model, forecast_start_date=time.strftime(str(datetime.datetime.now()), "%Y-%m-%d"), forceast_end_date=time.strftime(str(datetime.datetime.now()+datetime.timedelta(days=7)), "%Y-%m-%d")):
    ...
    forecast_days = date1 - date2
    horizon = 24*forecast_days
    future = model.make_future_dataframe(periods = horizon, freq='30min')
    forecast = model.predict(future)
    forecast_y_df = forecast[['ds', 'yhat', 'yhat_lower', 'yhat_upper']]
    print(forecast_days)
    return forecast_y_df
