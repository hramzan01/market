...
def create_train_test_set(file, d, previous_days=6*30):
    """
    AEOXLEY in progress. Returns original df, and train set
    ...
    # sample test set so it takes every other entry
    train = train.iloc[1:]
    train = train.iloc[::2,:]
    test = test.iloc[::2,:]
...
#    """
#    USe train test split from now onwards
...
def ml_model(train, forecast_days=7, seasonality_mode = 'multiplicative', year_seasonality_mode=4, freq='h'):
    ...
    y_pred = forecast_y_df.iloc[-168 :]

    return model, y_pred


#def ml_model_cv(train, forecast_days=7, seasonality_mode = 'multiplicative', year_seasonality_mode=4, freq='h'):
#    """Returns trained prophet model and forecasting for """
#    model = Prophet(seasonality_mode=seasonality_mode, yearly_seasonality=year_seasonality_mode, interval_width=0.95)
#    model.fit(train)
#    horizon = 24*forecast_days
...
def energy_model_run(date):
    ...
    train, test = create_train_test_set(file, d=date, previous_days=6*30)
    model, forecast_y_df = ml_model(train, forecast_days=7, seasonality_mode = 'multiplicative', year_seasonality_mode=4, freq='h')
    print(forecast_y_df.columns)
...
if __name__ == '__main__':
    ...
    test, forecast_y_df = energy_model_run(date)
